...
void showClose(const char * windowName, cv::Mat shape){
  imshow(windowName,shape);
  waitKey(0);
...
int main(int argc, char* argv[]){
  ...
  cv::Mat quad1 = cv::Mat::zeros(r.height, r.width, CV_8UC3);
  // Köşeleri bulma
  ...
  showClose("Perspektif Alınmış Hali",quad);
  ...
  showClose("Daire Merkezleri",quad);

  double averR = 0;
  vector<double> row;
  vector<double> col;

  //Interpolasyon için satır ve sütun bulma
  ...
  for(int i=0;i<row.size();i++){
    ...
    for(int j=0;j<col.size();j++){
      ...
      if(p>=0.3 && p>max){
        max = p;
        ind = j;
        //Seçili dairelerin çizimi
        circle( quad1, c, averR, Scalar(0,255,0), 3, 8, 0 );
      }
    }
  ...
  // Çıktı
  showClose("Bütün Daireler",quad);
  showClose("Seçili Daireler",quad1);  
  return 0;
}
